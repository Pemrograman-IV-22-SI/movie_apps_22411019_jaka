...
class EditGenre extends StatefulWidget {
  const EditGenre({super.key});
  static const routeName = '/edit-genre';
...
class _EditGenreState extends State<EditGenre> {
                ...
                : ElevatedButton(
                    onPressed: () {
                      if (genreController.text.isEmpty &&
                        ...
                        editResponse(id_genre!, genreController.text);
                      }
                    },
  ...
  void editResponse(int id, genre) async {
    try {
      ...
      Response response;
      response =
          await dio.put(editGenre + id.toString(), data: {"genre": genre});
